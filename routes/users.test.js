...
describe('GET /users/:username', function() {
	...
	test('not found if user not found', async function() {
		const resp = await request(app).get(`/users/nope`).set('authorization', `Bearer ${a1Token}`);
...
//***************************USERS APPLYING FOR JOBS...GOOD LUCK */

describe('POST/users/:username/jobs/:id', function() {
	test('works for user', async function() {
		const resp = await request(app).post(`/users/u1/jobs/2`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(200);
		expect(resp.body).toEqual({ applied: '2' });
	});

	test('works for admin', async function() {
		const resp = await request(app).post(`/users/u1/jobs/2`).set('authorization', `Bearer ${a1Token}`);
		expect(resp.statusCode).toEqual(200);
		expect(resp.body).toEqual({ applied: '2' });
	});

	test('doenst work for wrong user', async function() {
		const resp = await request(app).post(`/users/u2/jobs/2`).set('authorization', `Bearer ${u1Token}`);
		expect(resp.statusCode).toEqual(401);
	});
});
