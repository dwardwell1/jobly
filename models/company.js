...
class Company {
   ...
   **/
	...
	static async findFilter(filter) {
		//first make sure min is less than max employees
		...
		// create some functions to go over filter object and write proper sql code to add to query
		let holding = [];
		function maxEmployeesSQL() {
			if ('maxEmployees' in filter) {
				holding.push(`companies.num_employees <= ${filter.maxEmployees}`);
			}
		}
		function nameSQL() {
			if ('name' in filter) {
				filter.name = `'%${filter.name.toLowerCase()}%'`;
				holding.push(`LOWER(companies.name) LIKE ${filter.name}`);
			}
		}
		function minEmployeesSQL() {
			if ('minEmployees' in filter) {
				holding.push(`companies.num_employees >= ${filter.minEmployees}`);
			}
		}
		maxEmployeesSQL();
		nameSQL();
